import { ISettingRegistry } from '@jupyterlab/settingregistry';
import { Correxit } from './correxit';

const configuration = { token: '', url: '' };

export async function initialize(registry: ISettingRegistry | null) {
  let dispose = () => {};
  if (!registry) return dispose;
  try {
    const settings = await registry.load(Correxit.REGISTRAR);
    const reconfigure = () => {
      const composite = settings.composite as { token: string; url: string; };
      configuration.url = composite.url.replace(/\/+$/, '');
      configuration.token = composite.token;
    };
    settings.changed.connect(reconfigure);
    reconfigure();
    dispose = () => void settings.changed.disconnect(reconfigure);
  } catch (reason) {
    console.warn(Correxit.REGISTRAR, 'settings error', reason);
  }
  return dispose;
}

export const moodle: Correxit.Registrar = async (workbook, identifier) => {
  type Assignment = { duedate: number; id: number; name: string; };
  type Course = {
    assignments: Assignment[];
    fullname?: string;
    id: number;
    shortname?: string;
  };
  type User = {
    email?: string;
    fullname?: string;
    id: number;
    idnumber?: string;
    roles?: { shortname?: string }[];
    username?: string;
  };

  const { token, url } = configuration;
  if (!token || !url) return null;

  const endpoint = `${url}/webservice/rest/server.php`;
  const query = (action: string, params = '') =>
    `${endpoint}?wstoken=${encodeURIComponent(token)}` +
    `&wsfunction=${action}` +
    `&moodlewsrestformat=json${params}`;

  async function request<T>(action: string, params = ''): Promise<T> {
    const response = await fetch(query(action, params));
    if (!response.ok)
      throw new Error(`${action} failed (status ${response.status})`);

    const payload = await response.json();
    if (payload && typeof payload === 'object' && 'exception' in payload)
      throw new Error(payload.message as string || `${action} failed`);
    return payload as T;
  }

  const student = ({ roles = [] }: User): boolean =>
    !roles.length ||
    roles.some(({ shortname }) => shortname === 'student');
  const identify = ({ email, fullname, id, idnumber, username }: User) =>
    username || email || idnumber || fullname || `${id}`;
  const normalize = (records: User[]): string[] =>
    [...new Set(records.filter(student).map(identify))]
      .filter(Boolean)
      .sort((a, b) => a.localeCompare(b));
  const records: { courses: Course[] } = await request(
    'mod_assign_get_assignments'
  );
  const courses = records.courses
    .filter(({ assignments }) => assignments.length > 0)
    .sort(
      (a, b) =>
        a.id - b.id ||
        (a.fullname || a.shortname || '').localeCompare(
          b.fullname || b.shortname || ''
        )
    );

  const rosters = await Promise.all(
    courses.map(async ({ id }) => {
      const users: User[] = await request(
        'core_enrol_get_enrolled_users',
        `&courseid=${id}`
      );
      return [id, normalize(users)] as const;
    })
  );
  const roster = Object.fromEntries(rosters);
  const registered = courses.map(course => ({
    assignments: course.assignments
      .map(a => ({
        expiration: a.duedate ? a.duedate * 1000 : null,
        id: String(a.id),
        name: a.name,
        roster: roster[course.id] ?? []
      }))
      .sort(
        (a, b) =>
          Number(a.id || 0) - Number(b.id || 0) ||
          a.name.localeCompare(b.name)
      ),
    group: course.fullname || course.shortname || String(course.id)
  }));
  const selected = identifier.assignment;
  if (!selected)
    return registered.flatMap(({ assignments }) => assignments);

  const grouped = registered.find(({ assignments }) =>
    assignments.some(
      ({ id, name }) => id === selected || name === selected
    )
  );
  return grouped || registered.flatMap(({ assignments }) => assignments);
};
